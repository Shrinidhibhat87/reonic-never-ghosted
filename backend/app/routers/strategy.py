...
def generate_strategy(deal_id: int, session: Session = Depends(get_session)) -> StrategyResult:
    ctx = context_builder.build(session, deal_id)
    if ctx is None:
        raise HTTPException(status_code=404, detail="deal not found")

    result = get_engine().generate(ctx)
...
def revise_step(
    deal_id: int, order: int, body: ReviseIn, session: Session = Depends(get_session)
) -> RevisionResult:
    ctx = context_builder.build(session, deal_id)
    if ctx is None:
        raise HTTPException(status_code=404, detail="deal not found")
