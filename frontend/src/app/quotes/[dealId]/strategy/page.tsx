...
import { addAppointment, addNote, draftStep, generateStrategy, getDeal, reviseStep } from "@/lib/api";
import { navTo } from "@/lib/nav";
import type { EngineChoice, PersonalTouch, RevisionResult, Step, StrategyResult } from "@/lib/types";

const ENGINE_LABEL: Record<string, string> = {
  voltagent: "VoltAgent",
  python: "Python engine",
  fake: "Deterministic",
  "voltagent (fallback)": "VoltAgent (fallback)",
};
...
export default function StrategyPage() {
  ...
  // Sales-agent controls: which engine writes the plan + whether to ask it for
  // out-of-the-box creative plays. Changing either re-runs generation.
  const [engine, setEngine] = React.useState<EngineChoice>("voltagent");
  const [creative, setCreative] = React.useState(false);
  const [touches, setTouches] = React.useState<PersonalTouch[]>([]);
  ...
  React.useEffect(() => {
    getDeal(dealId)
      .then((d) => {
        setTouches((d.deal.personal_touches as PersonalTouch[]) ?? []);
        setFirstName(d.customer.name.split(" ")[0]);
      })
      .catch(() => {});
  }, [dealId]);
  ...
  const regenerate = React.useCallback(() => {
    setError("");
    setLoading(true);
    generateStrategy(dealId, { engine, creative })
      .then(setStrategy)
      .catch((e) => setError(String(e)))
      .finally(() => setLoading(false));
  }, [dealId, engine, creative]);
  ...
  async function onRevise(order: number) {
    ...
    setRevising(order);
    try {
      const result = await reviseStep(dealId, order, instruction, { engine, creative }); // targets the real step
    ...
    <div className="flex h-full w-full items-start bg-default-background">
      ...
      <div className="app-main flex grow shrink-0 basis-0 flex-col items-start self-stretch overflow-auto">
        <div className="flex w-full flex-wrap items-center gap-4 border-b border-solid border-neutral-border px-8 py-4 mobile:px-4">
          <Button variant="neutral-tertiary" icon={<FeatherArrowLeft />} onClick={() => navTo(router, `/quotes/${dealId}`, { back: true })}>
            {firstName || "Back"}
          </Button>
          <div className="flex grow shrink-0 basis-0 items-center gap-2">
            <span className="text-body font-body text-subtext-color cursor-pointer hover:text-default-font" onClick={() => navTo(router, "/quotes", { back: true })}>
              Quotes
            </span>
            <FeatherChevronRight className="text-body font-body text-subtext-color" />
            <span className="text-body font-body text-subtext-color cursor-pointer hover:text-default-font" onClick={() => navTo(router, `/quotes/${dealId}`, { back: true })}>
              Deal #{dealId}
            </span>
            <FeatherChevronRight className="text-body font-body text-subtext-color" />
        ...
        <div className="flex w-full flex-col items-start gap-6 px-8 py-8 mobile:px-4 mobile:py-6">
          {/* Sales-agent controls: pick the engine + opt in to creative plays. */}
          <div className="flex w-full flex-wrap items-end gap-4 rounded-lg border border-solid border-neutral-border bg-neutral-50 px-6 py-4">
            <Select
              className="h-auto"
              label="Strategy engine"
              value={engine}
              onValueChange={(v: string) => setEngine(v as EngineChoice)}
            >
              <Select.Item value="voltagent">VoltAgent (primary)</Select.Item>
              <Select.Item value="python">Python engine</Select.Item>
              <Select.Item value="fake">Deterministic</Select.Item>
            </Select>
            <Checkbox
              label="Think outside the box (creative plays)"
              checked={creative}
            ...
            {strategy && (
              <Badge variant="neutral">Generated by {ENGINE_LABEL[strategy.engine] ?? strategy.engine}</Badge>
            )}
          </div>

          {/* Add context the engine should weigh, then rebuild the whole plan. */}
          <div className="flex w-full flex-col items-start gap-2 rounded-lg border border-solid border-neutral-border bg-default-background px-6 py-4">
            <div className="flex items-center gap-1">
              <FeatherStickyNote className="text-body font-body text-brand-600" />
              <span className="text-body-bold font-body-bold text-default-font">Add context &amp; regenerate</span>
              <InfoTip text="New points are saved as a deal note and fed to the engine, then the full plan is rebuilt around them." />
            </div>
            <TextArea className="h-auto w-full flex-none" label="" helpText="">
              <TextArea.Input
                className="h-16 w-full flex-none"
                placeholder="e.g. she just told me a neighbour installed last month and loves it — lean on local proof."
                value={extraContext}
                onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setExtraContext(e.target.value)}
              />
            </TextArea>
            <Button icon={<FeatherWand />} loading={addingContext} disabled={!extraContext.trim()} onClick={onAddContext}>
              Add &amp; regenerate plan
            </Button>
          </div>
          ...
          {strategy && (
            <>
              ...
              <CreativePlays plays={strategy.creative_plays} />
              ...
              {touches.length > 0 && (
                <div className="flex w-full flex-col items-start gap-3 rounded-lg border border-solid border-brand-100 bg-brand-50 px-6 py-5">
                  <div className="flex items-center gap-2">
                    <FeatherGift className="text-heading-3 font-heading-3 text-brand-600" />
                    <span className="text-heading-3 font-heading-3 text-default-font">Personal touches in this plan</span>
                  </div>
                  <span className="text-caption font-caption text-subtext-color">
                    Gestures you picked on the prospect page — weave one in alongside the steps below.
                  </span>
                  <div className="flex w-full flex-wrap items-center gap-2">
                    {touches.map((t) => (
                      <div
                        key={t.id}
                        className="flex items-center gap-2 rounded-full border border-solid border-brand-300 bg-brand-100 px-3 py-1.5 text-caption font-caption text-brand-700"
                      >
                        <span>{t.emoji}</span>
                        <span>{t.label}</span>
                      </div>
